...
class LinfoOutput {
	...
	*/
	...
	public function htmlOut() {
		...
		if (isset($settings['theme']) && strpos($settings['theme'], '..') === false && file_exists('../../recursos/linfo/layout/theme_'.$settings['theme'].'.css'))
			$theme_css = 'theme_'.$settings['theme'].'.css';

		// Does default exist?? Don't bitch at me for assigning an array key in an if-then
		elseif (($settings['theme'] = 'default') && file_exists('../../recursos/linfo/layout/theme_'.$settings['theme'].'.css'))
			$theme_css = 'theme_'.$settings['theme'].'.css';
...
*/
	...
	echo '
	<link href="'.GetURL('recursos/linfo/layout/'.$theme_css).'" rel="stylesheet">'.( $show_icons ? '
	<link href="'.GetURL("recursos/linfo/layout/icons.css").'" rel="stylesheet">' : ''
	).'	
	...
	<![endif]-->
	<link rel="stylesheet" type="text/css" href="'.GetURL("recursos/linfo/layout/mobile.css").'" media="screen and (max-width: 640px)">
...
<div class="col2_side">
	...
	echo '
<div id="foot">
	'.sprintf($lang['footer_app'], round(microtime(true) - $this->linfo->getTimeStart(), 2)).'
</div>
<div id="foot_time">
...
';
/*
<script src="'.GetURL("recursos/linfo/layout/scripts.min.js").'"></script>
