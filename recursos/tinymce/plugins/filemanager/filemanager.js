...
//              break;
    ...
    // show the bubble
    if ($(window).width() - $(item).offset().left > 300) {
        $('.fmbubble').removeClass('fmbubRight').css({
            'right': 'auto',
            'left': $(item).position().left + 11 + $(item).width(),
            'top': $(item).position().top + 5
        }).show('blind', {direction: "horizontal"}, 500)
    } else {
        $('.fmbubble').addClass('fmbubRight').css({
            'left' : 'auto',
            'right': $(window).width() - $(item).position().left - 5,
            'top': $(item).position().top + 5
        }).show('blind', {direction: "horizontal"}, 500)
...
$(document).ready(function($) {
	...
	$(window.parent).resize(function(event) {
		...
		if (w > 800 ) {
			$('.mce-window[aria-label="My Files Home"]',window.parent.document).width( w - 40 ).children('.mce-container-body').width( w - 40 )
		} else {
			$('.mce-window[aria-label="My Files Home"]',window.parent.document).width(800).children('.mce-container-body').width(800)
			w = 800
		}
		var h = Number($(window.parent).innerHeight())
		if (w > 400 ) {
			$('.mce-window[aria-label="My Files Home"]',window.parent.document).height( h - 80 ).children('.mce-container-body').height( h - 80 )
		} else {
			$('.mce-window[aria-label="My Files Home"]',window.parent.document).height(400).children('.mce-container-body').height( 400 )
		}		
        $('.fmslider').width( w -100 )
		$('#toolbar, #fmtopbar').width( w -80 )
	}).trigger('resize')
	// =====================================================================
	// Change Basic CSS                                                     
	// =====================================================================
    ...
    $('.fmbubsm').click(function () {
        parent.tinymce.activeEditor.insertContent('<a class="thumbnail" href="' + $('.fmselect').children('img').attr('src').replace("/_thumb/", "/") + '" target="_blank"><img src="' + $('.fmselect').children('img').attr('src') + '" /></a>');
        $('#fmclose').click();
    });
    $('.fmbubmid').click(function () {
        parent.tinymce.activeEditor.insertContent('<a class="mediumimage " href="' + $('.fmselect').children('img').attr('src').replace("/_thumb/", "/") + '" target="_blank"><img src="' + $('.fmselect').children('img').attr('src').replace("/_thumb/", "/_middle/") + '" /></a>');
        $('#fmclose').click();
    });
    $('.fmbublg').click(function () {
        parent.tinymce.activeEditor.insertContent('<img src="' + $('.fmselect').children('img').attr('src').replace("/_thumb/", "/") + '" class="largeimage" />');
        $('#fmclose').click();
    });
   // Embad File
    $('.fmbubem').click(function () {
        ...
        switch (ext.toLowerCase()) {
        case 'pdf':
            parent.tinymce.activeEditor.insertContent('<iframe style="height:500px" src ="' + file + '" width="100%" class="innerimg"></iframe>');
            break;
        case 'swf':
            parent.tinymce.activeEditor.insertContent('<object width="320" height="240" data="'+file+'"><param name="src" value="'+file+'" /></object><br>');
            break;
        case 'mov':
            parent.tinymce.activeEditor.insertContent('<object width="320" height="240" data="'+file+'" type="video/quicktime"><param name="src" value="'+file+'" /></object><br>');
            break;
        case 'wmv':
            parent.tinymce.activeEditor.insertContent('<object width="320" height="240" data="'+file+'" type="application/x-mplayer2"><param name="url" value="'+file+'" /></object><br>');
            break;
        case 'mp3':
        case 'ogg':
            parent.tinymce.activeEditor.insertContent('<audio controls="controls"><source src="'+file+' " type="audio/mpeg" /></audio><br>');
            break;
        }
        $('#fmclose').click();
    });
    ...
    $('.fmbubdw').click(function () {
        parent.tinymce.activeEditor.insertContent('<a href="' + $('.fmselect').children('img').attr('data-wh') + $('.fmselect').children('img').attr('data-file') + '" target="_blank"> Download ' + $('.fmselect').text() + '</a>');
        $('#fmclose').click();
    });
    // Code Block
    $('.fmbubcode').click(function () {
        parent.tinymce.activeEditor.insertContent('<span class="cfmfile">' + $('.fmselect').children('img').attr('data-file') + '</span>');
        $('#fmclose').click();
    })
